...
// Sets the pins to be used for voltage and current sensors
//--------------------------------------------------------------------------------------
/* void EnergyMonitor::voltageExtADC(double _VCAL, double _PHASECAL)
{
  inPinV = _inPinV;
  VCAL = _VCAL;
  PHASECAL = _PHASECAL;
...
void EnergyMonitor::currentExtADC(double _ICAL)
{
  inPinI = _inPinI;
  ICAL = _ICAL;
  offsetI = ADC_COUNTS>>1;
...
void EnergyMonitor::calcVI(unsigned int crossings, unsigned int timeout)
{
  ...
  while(1)                                   //the while loop...
  {
    ...
    startV = ads1115.readADC_SingleEnded(inPinV);
  ...
  while ((crossCount < crossings) && ((millis()-start)<timeout))
  {
    ...
    //-----------------------------------------------------------------------------
    sampleV = ads1115.readADC_SingleEnded(inPinV);                 //Read in raw voltage signal
    sampleI = ads1115.readADC_SingleEnded(inPinI);                 //Read in raw current signal
...
double EnergyMonitor::calcIrms(unsigned int Number_of_Samples)
{
  ...
  {
    sampleI = ads1115.readADC_SingleEnded(inPinI);
